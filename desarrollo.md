...
### Ejercicio N°4:
Se modificó el servidor para que, al recibir la señal SIGTERM, cierre tanto la conexión actual con el cliente (`self._client_socket`) como el socket que utiliza para escuchar nuevas conexiones (`self._server_socket`). Además, se estableció el valor del atributo `running` en `False`, de modo que, una vez que se desbloquee el llamado a `accept()` (debido al cierre del socket), el bucle principal pueda finalizar correctamente y el programa termine de forma controlada.

En el caso del cliente, se implementó una goroutine que escucha la señal SIGTERM. Al recibirla, el cliente actualiza su atributo running a False, detiene el envío de mensajes y, si existe una conexión activa con el servidor, la cierra. Esto permite que el cliente finalice su ejecución de manera graceful, liberando correctamente los recursos utilizados.